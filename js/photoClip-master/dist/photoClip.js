...
(function(root, factory) {
	...
	$.fn.photoClip = function(option) {
		...
		var defaultOption = $.extend(options, option);

		new photoClip(this, defaultOption);
	...
	function photoClip(container, option) {
		var clipWidth = option.width,
			...
			file = option.file,
			ok = option.ok,
			fileMinSize = option.fileMinSize,
		...
		$file.change(function() {
			...
			if(!/image\/\w+/.test(this.files[0].type)) {
				...
				fileReader.onprogress = function(e) {
					if($.isFunction(loadProgress)) {
						loadProgress((e.loaded / e.total * 100).toFixed());
					}
				};
				fileReader.onload = function(e) {
					...
					} else if(kbs < fileMinSize) {
						if($.isFunction(imgSizeMin)) {
							imgSizeMin(kbs);
						}
					} else {
				...
				loadStart.call(fileReader, this.files[0]); // 开始加载图片
			}
		});
